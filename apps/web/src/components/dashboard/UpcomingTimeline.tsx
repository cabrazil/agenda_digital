  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <Link
          href="/calendar"
          className="text-[11px] font-bold flex items-center gap-1 text-indigo-400 hover:text-indigo-300 transition-colors"
        >
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 rounded-xl border border-dashed border-white/10 bg-white/[0.01]">
          ...
          <Link href="/kanban" className="mt-3 text-[11px] font-bold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors">
          ...
          {upcomingScheduled.map((c) => {
            ...
            return (
              <Link
                key={c.id}
                href={`/content/${c.id}`}
                className="group flex items-center gap-4 px-4 py-3 rounded-xl border border-white/[0.06] bg-white/[0.03] hover:bg-white/[0.06] hover:border-indigo-500/30 transition-all duration-200"
              >
                {/* Indicador de canal */}
                <div
                ...
                {/* Seta */}
                <ArrowRight size={14} className="flex-shrink-0 text-gray-600 group-hover:text-indigo-400 group-hover:translate-x-0.5 transition-all duration-200" />
              </Link>
            )
          })}
        </div>
